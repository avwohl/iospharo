...
// Out-of-line-dispatch B0 (docs/out-of-line-dispatch-design.md): when set,
// emitOne_arm64's return ops `b` to ONE shared J2J-return-prelude+epilogue
// block at method end instead of inlining it.  Set per-compile by
        ...
        if (probeThis) {
            ...
            if (nArgs == 1 && g_debug.t1InlinePrimBasicNew) {
                ...
                if (!GET_DEBUG_BOOL(PHARO_T1_NO_INLINE_NEW_ASM) && g_jitEdenFreeCell) {
                    ...
                    a.b(nbFill);
                    a.bind(nbFillDone);
                    a.str(x10, ptr(x11));          // commit edenFree_
                    a.stur(x9, ptr(x2, -16));      // result replaces class at sp[-2]
                    a.sub(x2, x2, asmjit::Imm(8)); // pop 1 net (popped class+size, pushed result)
                    emitStoreSp(a, x2);
                    ...
                    a.b(tosSendRes ? tosLrearm : endOfSend);
                    a.bind(nbBail);
                    // fall through to the C++ jit_rt_basic_new_with_arg path
                }
            ...
            if (nArgs == 0 && g_debug.t1InlinePrimBasicNew) {
                ...
                if (!GET_DEBUG_BOOL(PHARO_T1_NO_INLINE_NEW_ASM) && g_jitEdenFreeCell) {
                    ...
                    a.b_ne(inlBail);
                    a.ubfx(x17, x3, 0, 16);           // slotCount (instSize)
                    a.cmp(x17, asmjit::Imm(254));       // non-overflow cap
                    a.b_hi(inlBail);
                    a.ldr(x6, ptr(x1));               // class header
                    a.ubfx(x6, x6, 32, 22);          // classIndex = identityHash
                    ...
                    a.add(x15, x9, asmjit::Imm(8)); // first slot
                    a.mov(x16, x17);                // counter = slotCount
                    a.bind(inlFill);
    ...
    if (real) {
        ...
        for (size_t i = 0; i < bcRealLen; i++) {
            ...
            // single-byte ops fall through to emitOne_x86 below.
            {
                ...
                // to the interpreter, which has all the operand bytes and
                // executes the op + the rest of the method.  This is the
                // maximally-safe mirror of the arm64 loop's operand-stepping
                // (the divergence was that the x86 loop byte-stepped and fed
                // operand bytes to emitOne_x86 as opcodes).  Real inline x86
                // codegen for the hot ops (PushInteger, ExtPush, ...) is
                // layered on top once this baseline is verified.  Single-byte
                // ops fall through to emitOne_x86 below.
                //
                // Skip this op's full bytecodeLength: bind operand-byte
                // labels (so jumps don't dangle) and advance i by len-1 (the
                // loop's own i++ covers the opcode byte).  An ExtendA/B
