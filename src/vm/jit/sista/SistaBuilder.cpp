        ...
        if (calleeIR.values.size() == 2) {
            ...
            // Recursively inline the inner self-send.
                ...
                if (innerIR.values.size() == 2) {
                    ...
                    // Recursive chain: inner is itself a 2-value self-
                    // forwarder `^ self bar`.  Walk one more level using
                    // the new depth=3 budget — chain through to inner-
                    // inner if it's a const-return shape.
                    if (iv0.op == Op::kLoadReceiver
                     && iv1.op == Op::kSendUnspeculated
                     && !iv1.operands.empty() && iv1.operands[0] == iv0.id
                     && g_calleeLiftDepth < 3) {
                        uint32_t iNArgs = (uint32_t)((iv1.literal >> 16) & 0xFF);
                        uint32_t iBcOff = (uint32_t)(iv1.literal >> 24);
                        if (iNArgs != 0) return false;
                        std::vector<InlineHint> iHints =
                            g_hintProvider(innerOop);
                        const InlineHint* iHit = nullptr;
                        for (const auto& ih : iHints) {
                            if (ih.bcOffset == iBcOff) {
                                iHit = &ih; break;
                            }
                        }
                        if (!iHit) return false;
                        if (iHit->targetMethod == 0) return false;
                        if ((iHit->targetMethod & 0x7) != 0) return false;
                        if (iHit->targetMethod < 0x10000) return false;
                        // Inner-inner class must equal inner class
                        // (which already equals outer class).
                        if ((iHit->classOop & 0x3FFFFFu)
                            != (innerHit->classOop & 0x3FFFFFu)) return false;
                        Method iiIR;
                        uint32_t iiFailedAt = UINT32_MAX;
                        Oop iiOop = Oop::fromRawBits(iHit->targetMethod);
                        g_calleeLiftDepth++;
                        LiftResult iir;
                        {
                            ClearOuterHints g;
            ...
            // 3-value 1-arg self-forwarder: kLoadReceiver + kLoadTemp +
            // kSendUnspeculated.  Body is `^ self foo: arg`.  Inline:
            // outer guard on caller's recv (same as outer hint), no
            // inner guard (inner rcvr IS self), inline inner const-
            // return body — but only when inner body doesn't reference
            // the arg (kLoadTemp absent).  Args-referencing inner
            // bodies need parameter substitution (kInlineSend Phase 4).
            // Second-most unrecognized shape (sz=3/02.06.13.00:46).
            else if (v0.op == Op::kLoadReceiver
                  && v1.op == Op::kLoadTemp
                  && v2.op == Op::kSendUnspeculated
                  && v2.operands.size() >= 2
                  ...
                  && v2.operands[1] == v1.id) {
                if (!g_hintProvider) return false;
                uint32_t tempIdx = static_cast<uint32_t>(v1.literal);
                if (tempIdx >= nArgs) return false;
                uint32_t innerNArgs = (uint32_t)((v2.literal >> 16) & 0xFF);
                uint32_t innerBcOff = (uint32_t)(v2.literal >> 24);
            ...
            // 3-value 1-arg self-forwarder w/ const arg: kLoadReceiver +
            // kConstantOop + kSendUnspeculated.  Body `^ self foo: K`.
            // Inline same as the kLoadTemp variant above (inner rcvr is
            // self, no inner guard needed because inner class == outer)
            // but inner can also reference its arg via kLoadTemp(0) —
            // if so, substitute with the constant K.  4th-most
            // unrecognized shape (sz=3/02.00.13.00:25).
            else if (v0.op == Op::kLoadReceiver
                  && v1.op == Op::kConstantOop
                  && v2.op == Op::kSendUnspeculated
                  && v2.operands.size() >= 2
                  && v2.operands[0] == v0.id
                  && v2.operands[1] == v1.id) {
                ...
                g_calleeLiftDepth--;
                if (ir != LiftResult::kOk) return false;
                // Inner shape: either 2-value (const-return) or
                // 2-value `kLoadTemp(0) + kReturn` (arg passthrough →
                // result is the const we passed).
                if (innerIR.values.size() != 2) return false;
                const Value& iv0 = innerIR.values[0];
                const Value& iv1 = innerIR.values[1];
                if (iv1.op != Op::kReturn) return false;
                ...
                case Op::kLoadReceiver: inlineOp = Op::kLoadReceiver;
                                        inlineTy = Type::kOop; break;
                case Op::kLoadTemp: {
                    // `^ arg` — inline value is the constant K we passed.
                    if (static_cast<uint32_t>(iv0.literal) != 0) return false;
                    inlineOp = Op::kConstantOop;
                    inlineTy = Type::kOop;
                    inlineLit = v1.literal;
                    break;
                }
