...
enum class Op : uint8_t {
    ...
    // jit-may23e Eβ: inline check for `^ (self bitAnd: 1) = 0/1`
    // (Integer>>even / >>odd).  Generated by SistaBuilder when a
    // Send1 target's bytecode matches the even/odd shape.  Lowering:
    //   tag-check operand (SmI), bits & 9 vs 1 (even) or 9 (odd),
    //   csel trueOop/falseOop.  Bail to kSendUnspeculated on miss.
    // operands: [receiver]
    // literal:  bit 0 = kind (0=even, 1=odd)
    //           bits 16-47 = bcOffset (for deopt)
    kPrimEvenOddCheck,
