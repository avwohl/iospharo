...
size_t JITRuntime::drainInitialCompileQueue() {
    ...
    for (size_t i = 0; i < head; i++) {
        ...
        if (!compiler_->compile(method)) {
            ...
            // initialCompileFailed comment in JITRuntime.hpp.
            initialCompileFailedInsert(methBits);
        }
        processed++;
        g_initialCompileQueueDrained++;
...
void JITRuntime::noteMethodEntry(Oop compiledMethod) {
    ...
    // backward jumps (bytecode-level loops).  Scan only on the path
    ...
    for (size_t probe = 0; probe < 8; probe++) {
        ...
        if (countMap_[i].key == 0) {
            countMap_[i].key = key;
            countMap_[i].count = 1;
            // jit-may22b Step 13: hot-loop methods compile immediately.
            if (threshold <= 1 || methodHasBackwardJump()) {
                goto compile_check;
            }
            return;
        }
...
void JITRuntime::recoverAfterGC(ObjectMemory& memory) {
    ...
    // 2026-05-03: IC zone moved to heap; no W^X flip required.
    {
        ...
        const bool staleLog = g_debug.jitStaleLog;
    ...
    // are stale after a move — rehash, same as countMap_ below.  (Was a
    // clear() before 2026-06-10, which re-ran the asmjit pipeline on every
    // failing method once per GC: 1.4M+ failed compile attempts per
    // 60-class SUnit batch, ~700/sec of pure codegen churn.)
    rehashInitialCompileFailed();
