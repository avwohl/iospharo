...
void Interpreter::sendSelector(Oop selector, int argCount) {
    ...
    // The issue: determineActivePlatform returns OrderedCollection instance instead of class
    {
        ...
        if (selEquals("detect:ifNone:") && detectTraceCount++ < 30) {
            Oop rcvr = stackValue(argCount);  // Receiver (the collection)
            Oop aBlock = stackValue(1);       // First arg (the detection block)
            Oop ifNoneBlock = stackValue(0);  // Second arg (the ifNone block)
            ...
            if (aBlock.isObject() && aBlock.rawBits() > 0x10000) {
                ObjectHeader* blkHdr = aBlock.asObjectPtr();
                uint32_t blkClassIdx = blkHdr->classIndex();
                Oop blkClass = memory_.classAtIndex(blkClassIdx);
                if (blkClass.isObject() && blkClass.rawBits() > 0x10000) {
                    Oop blkClsName = memory_.fetchPointer(6, blkClass);
                    if (blkClsName.isObject() && blkClsName.rawBits() > 0x10000) {
                        ObjectHeader* bcnHdr = blkClsName.asObjectPtr();
        ...
        // Trace detect:ifFound:ifNone: to see what aBlock is received
        if (selEquals("detect:ifFound:ifNone:")) {
            static int detectFound3TraceCount = 0;
            if (detectFound3TraceCount++ < 20) {
                // argCount=3: receiver at stackValue(3), aBlock at stackValue(2), etc.
                Oop rcvr = stackValue(argCount);
                Oop aBlock = stackValue(2);  // First arg after receiver
                std::cerr << "[DETECT3-TRACE #" << detectFound3TraceCount << "] detect:ifFound:ifNone: aBlock=";
                if (aBlock.isObject() && aBlock.rawBits() > 0x10000) {
                    ObjectHeader* blkHdr = aBlock.asObjectPtr();
                    uint32_t blkClassIdx = blkHdr->classIndex();
                    Oop blkClass = memory_.classAtIndex(blkClassIdx);
    ...
    if constexpr (ENABLE_DEBUG_LOGGING) {
        ...
        // Trace process termination: log when terminate/doTermination methods are sent
        {
            auto selMatch = [&](const char* s) {
                size_t n = strlen(s);
                return selLen == n && selBytes && memcmp(selBytes, s, n) == 0;
            };
            if (selMatch("terminate") || selMatch("doTerminationFromYourself") ||
                selMatch("doTerminationFromAnotherProcess") || selMatch("unwindTo:") ||
                selMatch("unwindAndStop:") || selMatch("resume:through:") ||
                selMatch("resume:value:through:") || selMatch("endProcess") ||
                selMatch("cannotReturn:") || selMatch("reachedDefaultHandler:") ||
                ...
                if (procTermLog && procTermLogCount++ < 200) {
                    ...
                    if (rc.isObject()) {
                        Oop nm = memory_.fetchPointer(6, rc);
                        if (nm.isObject() && nm.rawBits() > 0x10000) {
                            ObjectHeader* nmH = nm.asObjectPtr();
                            if (nmH->isBytesObject() && nmH->byteSize() < 50)
                                rcvrClass = std::string((char*)nmH->bytes(), nmH->byteSize());
                    ...
                    std::string selName(selBytes ? selBytes : "?", selBytes ? selLen : 1);
                    fprintf(procTermLog, "[TERM-SEND #%d step=%llu] #%s rcvr=0x%llx(%s) activeProc=0x%llx(p%d) args=%d\n",
                            procTermLogCount, (unsigned long long)g_stepNum,
                            selName.c_str(), (unsigned long long)rcvr.rawBits(), rcvrClass.c_str(),
                            (unsigned long long)ap.rawBits(), prio, argCount);
                    // Also log the current method for context
                    ...
                    fprintf(procTermLog, "  from method=#%s\n", curMethod.c_str());
                    fflush(procTermLog);
                }
            }
        }
            ...
            if (selStr == "error:" && argCount == 1) {
                ...
                if (errorMsg.find("Invalid option name") != std::string::npos) {
                    ...
                    if (optLog) {
                        ...
                        // Dump ALL context slots (not just 6-14)
                        auto dumpOop = [&](FILE* f, const char* label, Oop o) {
                            if (o.isSmallInteger()) {
                                fprintf(f, "%s=SmallInt(%lld)", label, o.asSmallInteger());
                            } else if (o.isNil()) {
                                fprintf(f, "%s=nil", label);
                            } else if (o.isImmediate()) {
                                fprintf(f, "%s=imm(0x%llx)", label, (unsigned long long)o.rawBits());
                            } else if (o.isObject() && memory_.isValidPointer(o)) {
                                ObjectHeader* h = o.asObjectPtr();
                                ...
                                if (h->isBytesObject() && h->byteSize() < 100) {
                                    fprintf(f, "%s='%.*s'(%s@0x%llx)", label, (int)h->byteSize(), (char*)h->bytes(),
                                            cn.c_str(), (unsigned long long)o.rawBits());
                                } else {
                                    fprintf(f, "%s=%s(slots=%zu,fmt=%u@0x%llx)", label, cn.c_str(),
                                            h->slotCount(), (unsigned)h->format(), (unsigned long long)o.rawBits());
                                }
                            } else {
                                fprintf(f, "%s=???(0x%llx)", label, (unsigned long long)o.rawBits());
                            }
                        };
                                    ...
                                    if (nLits >= 2 && nLits < 100) {
                                        ...
                                        if (sel.isObject() && memory_.isValidPointer(sel)) {
                                            ...
                                            if (selH->isBytesObject() && selH->byteSize() < 100) {
                                                ...
                                                fprintf(optLog, "  [%d] #%s\n", d, selName.c_str());
                                                // Dump ALL slots for interesting contexts
                                                if (selName == "parseOptions:" || selName == "options:" || selName == "caseOf:otherwise:") {
                                                    ObjectHeader* ctxH = ctx.asObjectPtr();
                                                    size_t ctxSlots = ctxH->slotCount();
                                                    fprintf(optLog, "    Context slots=%zu receiver=", ctxSlots);
                                                    Oop rcvr = memory_.fetchPointer(5, ctx);
                                                    dumpOop(optLog, "", rcvr);
                                                    fprintf(optLog, "\n");
                                                    for (size_t s = 6; s < std::min(ctxSlots, (size_t)25); s++) {
                                                        Oop slot = memory_.fetchPointer(s, ctx);
                                                        char lbl[16]; snprintf(lbl, sizeof(lbl), "    [%zu]", s);
                                                        dumpOop(optLog, lbl, slot);
                                                        fprintf(optLog, "\n");
                                                    }
                                                }
                                                // For parseOptions:, also dump the option value specifically
                                                if (selName == "parseOptions:") {
                                                    // Slot 6 = optionsArray arg, slot 7 = include, slot 8 = option, slot 9 = rs
                                                    fprintf(optLog, "    --- parseOptions: breakdown ---\n");
                                                    Oop optArray = memory_.fetchPointer(6, ctx);
                                                    dumpOop(optLog, "    optionsArray", optArray);
                                                    fprintf(optLog, "\n");
                                                    if (optArray.isObject() && memory_.isValidPointer(optArray)) {
                                                        ObjectHeader* ah = optArray.asObjectPtr();
                                                        for (size_t i = 0; i < std::min(ah->slotCount(), (size_t)10); i++) {
                                                            Oop elem = ah->slotAt(i);
                                                            char lbl[32]; snprintf(lbl, sizeof(lbl), "    elem[%zu]", i);
                                                            dumpOop(optLog, lbl, elem);
                                                            fprintf(optLog, "\n");
                                                        }
                                                    }
                                                    dumpOop(optLog, "    option(slot8)", memory_.fetchPointer(8, ctx));
                                                    fprintf(optLog, "\n");
                                                }
                                            }
                                        }
