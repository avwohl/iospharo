...
DEBUG_INT(PHARO_CANNOT_RETURN_BURST, 64)   // cannotReturnStormGuard: how many cannotReturn: sends one process may make inside a 2,000,000-step window before the VM terminates it.  Default 64 -- an image handler that resumes a legitimate BlockCannotReturn sends a handful, a defect-#23 storm sent 515,981.  Lower it to exercise the guard (it never fires in a healthy run, so this is the only way to test the path).
DEBUG_BOOL(PHARO_J2J_SITE5_CLEAR_DEPTH)   // opt-IN: after the ExitJ2JCall handler's materialize ("site5"), clear state.j2jDepth and re-base the save cursor the way the materializeJ2J lambda does.  The handler otherwise `continue`s with the depth still standing and the cursor un-rebased, and enableJ2J() (which would rebase) is not on that path -- a real inconsistency.  The duplicate-activation evidence that motivated it was RETRACTED (the detector was flagging a region rebuild at "j2jBase materialize").  MEASURED 2026-09-03, 10 storm reps: guard fires 6/10 with it against 7/10 without, both knobs together 8/10 -- i.e. no effect.  Clean on the batch-1-100 smoke test.  Kept inert because the code reading still stands.
DEBUG_BOOL(PHARO_T1_J2J_EXCLUDE_BLOCK_CREATE)  // opt-IN: refuse cross-method inline-J2J for callees whose body contains PushFullBlock/PushClosure.  A block created while J2J saves are pending captures an outerContext missing every J2J-hidden caller (defect #23, 481 such creations in a 20-second run).  MEASURED 2026-09-03, 10 storm reps under held load: aborts 0/10 either way, cannotReturn:-guard fires 5/10 with it against 7/10 without -- a hint, not a fix, and it costs inlining on block-heavy code.  Clean on the batch-1-100 smoke test.  Stays opt-in.
...
DEBUG_STR(PHARO_WIDOW_TRACE_SEL)            // print every context WIDOWED on return whose method selector contains this substring, tagged with the site (popFrame / popFrameForJIT / ctx-return). Answers "was this activation ever widowed at all?", which is the question behind a retained-receiver chain (docs/vm-compat-bugs.md #28). Bounded to 200 lines.
DEBUG_BOOL(PHARO_NO_JIT_WIDOW_ON_POP)       // opt-OUT (default ON): popFrameForJIT widows the returning frame's materialized Context (sender=nil, pc=nil) exactly as popFrame() does. Without it a returned activation's Context keeps a live sender chain, and a closure captured out of that frame (outerContext) pins every receiver in the chain -- measured as 71 Context:start + 71 Context:privateStart heap parents holding dropped TKTWorkers with the JIT on against 0 with it off (docs/vm-compat-bugs.md #28). Set this to restore the leak for a bisect.
