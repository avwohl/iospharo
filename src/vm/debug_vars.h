...
DEBUG_BOOL(PHARO_OLDSPACE_FREELIST)   // EXPERIMENTAL AND CURRENTLY BROKEN -- do not enable except to work on it. After compaction, puts the gaps BELOW oldSpaceFree (stranded by pinned objects that sliding compaction cannot move) on the free list and lets old-space allocation reuse them. The gap accounting works ([GC-FREELIST] reports 17 gaps / 148 MB on a NeoJSON image) but a load STALLS with it on. 2026-08-23: the stall is an INFINITE WALK of a cyclic large-chunk list inside allocateFromFreeList -- frozen at exactly 17,520 bytecode steps at 99% CPU, every sample in that function via primitiveNewWithArg->allocateBytes. One cycle source is fixed (the unlink stored a raw native pointer into a heap slot read back as an encoded Oop, so a mid-list removal wrote 0 = heap base = self-loop) and a Floyd guard now reports [FREELIST-CYCLE] instead of hanging, but ANOTHER source remains: the guard still fires and the load still does not finish. Next: find who else builds the list (rebuildFreeListAfterCompact) and whether a chunk is added twice or with an uninitialised link. Without it those gaps are never reused: 235 MB of old space holding 89 MB of objects (NeoJSON), 1.15 GB holding 94 MB (XMLParser). See docs/gc-oldspace-fragmentation-2026-08-22.md
