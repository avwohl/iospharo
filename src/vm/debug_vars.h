...
DEBUG_BOOL(PHARO_J2J_SITE5_CLEAR_DEPTH)   // opt-IN: after the ExitJ2JCall handler's materialize ("site5"), clear state.j2jDepth and re-base the save cursor the way the materializeJ2J lambda does.  Without it the handler `continue`s with the depth still standing, so the next materialize walks the SAME saves again -- a duplicate activation (observed: #methodNode, 27 times in one sweep).  Opt-in until A/B'd; the lambda's comment says leaving the marker set is what produced the 2026-06-09 xmethod-at-scale corruption.
DEBUG_BOOL(PHARO_T1_J2J_EXCLUDE_BLOCK_CREATE)  // opt-IN: refuse cross-method inline-J2J for callees whose body contains PushFullBlock/PushClosure.  A block created while J2J saves are pending captures an outerContext missing every J2J-hidden caller (defect #23, 481 such creations in a 20-second run).  Costs inlining on block-heavy code, hence opt-in until A/B'd.
