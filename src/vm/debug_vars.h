...
DEBUG_BOOL(PHARO_T1_AO_MAT_J2J)              // default-OFF fix for the open shared chain-loop bug: case ExitArithOverflow (Interpreter.cpp:26322) returns false WITHOUT materializing pending J2J saves when j2jDepth>0, so a method entered via cross-method inline-J2J that arith-overflows resumes in the interp with its CALLER frame leaked (only in the unpopped save) -> corruption. When set, the handler materializes (the in-scope materializeJ2J lambda) before return. APPENDED at END of list to confine DebugBoolId index churn to this one knob. Knob-OFF short-circuits (GET_DEBUG_BOOL false) -> body never runs -> dead-branch behavior-identical to the legacy handler (NOT byte-identical: one never-taken branch is added; capstone-diff WILL show it — expected, not a regression). x86/V1 is the validated config (V1 save holds save.jitMethod=CALLER); arm64 is V2 (findMethodByPC path) and ships knob-OFF. Deterministic repro: PHARO_X86_JIT=1 PHARO_T1_X86_XMETHOD=1 PHARO_JIT_FAIL_REASONS=1 eval "(3+4) printString". docs/x86-xmethod-j2j-design.md.
