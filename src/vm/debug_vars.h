...
DEBUG_BOOL(PHARO_T1_AO_MAT_J2J)              // default-OFF, x86/V1-scoped (#if !PHARO_J2J_SAVE_V2): case ExitArithOverflow (Interpreter.cpp:26322) materializes pending J2J saves (j2jDepth>0) via the in-scope materializeJ2J lambda before `return false`. NECESSARY-BUT-INSUFFICIENT (box-validated 2026-06-15): with PHARO_T1_X86_XMETHOD=1, enabling this does NOT fix the startup corruption ((3+4) still nil, like the control) — so the AO bail is one leak but not the complete cause (other j2jD>0 bail exits during startup, or the materialize mis-reconstructs the x86 save buffer). Next: AO-MAT-PROBE (print save0.jitMethod vs state.jitMethod + post-materialize j2jDepth/frameDepth) and count NON-AO Exit codes with j2jD>0. arm64-SAFE: compiles out on V2 (knob-OFF AND knob-ON both battery==golden, verified). docs/x86-xmethod-j2j-design.md.
