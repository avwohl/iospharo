...
DEBUG_BOOL(PHARO_T1_X86_EMIT_EXTSEND)       // x86 emit-polish (opt-in, default-OFF): emit a NAKED ExtSend (0xEA, no ExtendA/B prefix) as a real in-JIT cached send (own IC slot) instead of a mid-method bail-to-interp, so ExtSend-bearing methods stay fully JIT'd.  inline-J2J is suppressed at ExtSend sites (the 2-byte op resumes at globalIdx+2 via bcToCode, not the 1-byte resume override); the IC-probe + getter/setter/returnsSelf + dispatchCached paths emit normally.  Gates the WHOLE feature; OFF = byte-identical default JIT (isX86SendSite reduces to isPhase4SendOp).  x86-only emit (arm64 compiles out the predicate).  Prefixed (ExtendA/B+ExtSend) ExtSend still bails.  docs/x86-nested-j2j-design.md.
...
DEBUG_BOOL(PHARO_T1_X86_XMETHOD_SENDS)      // Increment 2 (opt-IN, KNOWN-BROKEN as of box #19 2026-06-16): admit SEND-BEARING cross-method inline-J2J callees (numICEntries>0, up to PHARO_T1_XMETHOD_MAX_IC), replacing the canSkipJ2JSave (numIC==0 leaf-only) gate. CORRUPTS: cfibt (=cfibt body calling inct=^(self+1) incc) computes WRONG under this knob — 787M bytecode steps for cfibt(28) (vs correct ~6M), actChain=11, results vanish (no EVAL-RESULT). Naive gate-relaxation is INSUFFICIENT: the leaf-only gate guards a real NESTED cross-method J2J save/resume bug (cfibt->inct->incc, three distinct methods: the inner send's save/resume on top of the caller's cross-method save corrupts the frame/save-stack). SENDS=0 (default) is correct + fast-where-applicable (cfibx 71ms). The real fix is the cross-method-send-bearing callee frame/materialize correctness (needs lldb on box; arm64's path handles it). Kept as a REPRODUCER for that work. arm64 compiles out (x86-only emit). docs/x86-cog-gap.md.
