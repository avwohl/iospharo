...
DEBUG_BOOL(PHARO_T1_X86_XMETHOD)            // Increment 1 (2026-06-15): cross-method (caller!=callee) inline-J2J on x86, restricted to callee->canSkipJ2JSave callees (no send + no cond-jump bail) so only the clean return-prelude path runs. Default OFF; knob-off => byte-identical self-rec-only emit. See docs/x86-xmethod-j2j-design.md. (Increment 2 = bailmid callees is BLOCKED on the open ExitArithOverflow-materialize bug.)
