...
DEBUG_BOOL(PHARO_T1_SHARED_RETPRELUDE)    // out-of-line-dispatch B0 (docs/out-of-line-dispatch-design.md): when an arm64 method has >=2 prelude-using return ops, route them to ONE shared J2J-return-prelude+epilogue block at method end (b sharedLabel) instead of inlining ~17 insns per return.  Single-return methods stay inline (no regression).  Opt-in; knob-off is byte-identical.  Zone/i-cache win, latency-neutral.
