...
DEBUG_BOOL(PHARO_T1_X86_XMETHOD_ALLARGS)    // opt-IN: admit nArgs>0 cross-method inline-J2J callees. Default OFF = nArgs==0 (unary) cross-method only. The nArgs>0 path corrupts the CALLER's receiver at startup scale (box 2026-06-15: String class>>new:streamContents: dispatched #new: on Symbol class -> ShouldNotImplement; cfibx never caught it because its only cross-method callee, incc, is nArgs==0). Unary cross-method (getters, class, size, isNil...) is the bulk of real sends and is the safe default-on subset. Set to re-test the broken nArgs>0 path. arm64 compiles out (x86-only static).
