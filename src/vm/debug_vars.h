...
DEBUG_BOOL(PHARO_T1_NO_INLINE_NEW_ASM)      // opt-OUT (default ON 2026-06-20): inline the basicNew (0-arg, fixed-size pointer class) eden allocation directly in tier-1 emitted asm (AsmjitT1.cpp:8610), skipping the jit_rt_basic_new -> jitBasicNew -> primitiveNew C++ call chain. Reads instSpec from the class receiver at runtime; verifies classTable[identityHash]==class; bails to the C++ helper for variable/overflow/eden-full/non-fixed/hash==0/non-canonical-index. GC-safe (eden bump inits header+slots before committing edenFree_; no safe point mid-init). Set this to force the C++ helper path.
