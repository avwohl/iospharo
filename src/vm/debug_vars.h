...
DEBUG_BOOL(PHARO_T1_INLINE_NEW_ASM)         // opt-IN (default OFF, WIP 2026-06-20): inline the basicNew (0-arg, fixed-size pointer class) eden allocation directly in tier-1 emitted asm (AsmjitT1.cpp:8610), skipping the jit_rt_basic_new -> jitBasicNew -> primitiveNew C++ call chain. Reads instSpec from the class receiver at runtime; bails to the C++ helper for variable/overflow/eden-full/non-fixed/hash==0. GC-safe (eden bump inits before committing edenFree_; no safe point mid-init). The big "as-fast-as-Cog" allocation lever. Validate A/B (knob-on == knob-off) before any default flip.
