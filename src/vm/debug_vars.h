...
DEBUG_BOOL(PHARO_T1_NO_INLINE_NEW_ASM)      // opt-OUT (default ON 2026-06-20): inline eden allocation directly in tier-1 emitted asm for basicNew (0-arg, fixed-size pointer class, AsmjitT1.cpp:8610) AND basicNew: (1-arg, pointer-indexable Array new:N, format 2 fixedSize 0), skipping the jit_rt_basic_new[_with_arg] -> jitBasicNew -> primitiveNew C++ call chain. Reads instSpec from the class receiver at runtime; verifies classTable[identityHash]==class; bails to the C++ helper for variable-with-fixed/bytes/words/overflow(>=254)/eden-full/non-fixed/negative-size/hash==0/non-canonical-index. GC-safe (eden bump inits header+slots before committing edenFree_; no safe point mid-init). slotCount/size in x17 (NOT x5 = icDataPtr, which dispatchCached needs on the bail path). Set this to force the C++ helper path.
