...
DEBUG_BOOL(PHARO_T1_AO_MAT_J2J)              // default-OFF, x86/V1-scoped (#if !PHARO_J2J_SAVE_V2): case ExitArithOverflow (Interpreter.cpp:26322) materializes pending J2J saves (j2jDepth>0) via the in-scope materializeJ2J lambda before `return false`. Addresses a REAL but SECONDARY bug (an inline-J2J callee that arith-OVERFLOWS leaks its caller frame). NOT the cure for the PHARO_T1_X86_XMETHOD startup corruption: workflow wf_8b29daf3 proved (3+4)=7 is in SmallInteger range so it NEVER reaches ExitArithOverflow — the (3+4)->nil corruption is the cross-method NORMAL send/return EMIT (blocker #1), independent of this AO path. The AO-MAT-PROBE confirmed this fix's materialize is CORRECT (save0.jm=CALLER, j2jD->0, fd++), so the save-read/reconstruction is right; it just isn't the (3+4) corruptor. Sufficiency for the AO case itself is UNVERIFIED (masked by blocker #1). arm64-SAFE: compiles out on V2 (knob-OFF AND knob-ON both battery==golden). docs/x86-xmethod-j2j-design.md.
