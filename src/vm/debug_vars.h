...
DEBUG_BOOL(PHARO_OLDSPACE_FREELIST)   // EXPERIMENTAL AND CURRENTLY BROKEN -- do not enable except to work on it. After compaction, puts the gaps BELOW oldSpaceFree (stranded by pinned objects that sliding compaction cannot move) on the free list and lets old-space allocation reuse them. The gap accounting works ([GC-FREELIST] reports 17 gaps / 148 MB on a NeoJSON image) but a load STALLS with it on. 2026-08-23: the stall is FIXED and the list is now correct, but the path is TOO SLOW to enable by default. Three bugs, all pointer discipline: the unlink stored a raw native pointer into a heap slot read back as an encoded Oop; allocateFromFreeList returned the shifted `chunk` (base+8 for overflow chunks) where the caller expects the allocation base; and every link reader used `next.isObject()`, but NIL IS AN OBJECT, so the terminator decoded to heap base (0x7000000000) whose own link is nil -- the self-loop. All link reads now go through freeListLinkToChunk(), and a build-time validator reports 'acyclic, 8 links, 8 gaps'. Measured with it on: the allocation workload went 240s-timeout -> 3s, a NeoJSON load 900s-timeout -> 20s, VM binaries pass both ways. BUT a 300-class SUnit batch takes 545s with it on against 139s off (4x), with 3 extra ClassTest timeouts caused by that slowdown -- 0 fail / 0 error both ways. The remaining work is PERFORMANCE, and it is NOT the allocator and NOT per-bytecode cost. Measured on the same 300-class batch: the knob-on run executes 106,207,009,260 bytecodes against 25,144,386,021 with it off -- 4.2x MORE WORK, matching the 4.2x wall clock (545s vs 139s) almost exactly, with identical pass counts (4858/4861) and fullGC=4 vs fullGC=0. Profiling agrees the allocator is innocent: allocateFromFreeList is 1 sample in 4240 and all ObjectMemory frames ~300, the rest ordinary interpretation. So the VM is running 4x more SMALLTALK, not the same work more slowly, and the extra full GCs are the most likely engine (each one drives image-side finalization/weak processing). LOCALISED: the work is almost all in ClassTest, and it is enormous. Run ClassTest ALONE (printf 'ClassTest\n' > /tmp/sunit_class_names.txt): knob off finishes in 4 s with 862,660,125 bytecodes and all 44 tests; knob on is still running at 900 s having executed 479,672,900,248 -- ~556x -- with fullGC=4 vs 0 and ZERO free-list cycles, so this is not the corruption bug returning. ClassTest mutates class SHAPE (addInstVarName/addSlot), which migrates every instance and recompiles methods, so the suspicion is that reusing gaps perturbs instance migration or the enumeration it walks (free chunks ARE headered objects with classIndex 0). MINIMAL REPRODUCER (10 lines, no test framework): create a class and add 8 instance variables --  ((Object << #ZZShapeProbe) package: 'ZZProbe') install, then 1 to: 8 do: [ :i | cls addInstVarNamed: 'v', i printString ]. Knob off: 421 ms. Knob on: the 8 calls do not finish in 240 s (the eval exits 71). So it IS class-shape mutation / instance migration, not the test framework. AND THE SYMPTOM IS A WEDGE, NOT SLOWNESS: sampling the reproducer mid-run shows the VM at 1% CPU with 2054 of ~2072 samples in primitiveRelinquishProcessor, i.e. idle. The huge bytecode counts reported for ClassTest (479 G) are the runner's watchdog spinning on relinquishProcessor at ~190 M steps/s while the test process sits stuck -- a busy waiter, not real work. The process is not waiting -- it is KILLED: [DNU] CASCADE rcvr=0x700008ba20 classIdx=0 origSel=#isReadOnlyObject method_=#copyObject:to: under Metaclass>>addSlot:, and that address is byte-identical to node 7 of the free list dumped after rebuild. So the image touches a FREE CHUNK during instance migration. Both VM enumeration paths now skip classIndex 0 (findAccessibleObjectIn already did; allObjectsDo was fixed 2026-08-23), so the image is NOT enumerating it -- it holds a STALE REFERENCE to memory that was put on the free list while still reachable. Ruled out by test, do not re-try: skipping free chunks in allObjectsDo, and the prevEnd base-vs-header overshoot in rebuildFreeListAfterCompact (both fixed as correctness, neither cures it). ROOT CAUSE NARROWED 2026-08-23: logging every gap range shows the DNU receiver IS a gap chunk, at its exact start -- [FREELIST-GAP] 0x700008ba20..0x700008bc38 (536) vs CASCADE rcvr=0x700008ba20 -- and the identical gap list reappears after the NEXT fullGC, so the region is free again after each compaction. The gaps are verifiably all-zero, the chunks are correctly linked, and both enumeration paths skip classIndex 0. The only consistent explanation left: an object allocated from a low gap is RELOCATED by sliding compaction and at least one reference to it is not updated, leaving the image holding the old address -- which by then is a free chunk again. So the defect is in COMPACTION/FORWARDING for objects allocated BELOW the destination finger, not in the free list. That is also why the knob has never been usable: it is the first code path that puts live objects below the finger. Look at planCompactSavingForwarders and who updates references for objects it moves -- BUT note two further results. (a) Objects allocated from the free list survive a full GC perfectly: allocate 200 Arrays, hold them from a global, garbageCollect, and all 200 are intact (kept=200 intact=200), so marking/collection/relocation are fine for ordinary allocation. (b) The failure is therefore specific to class-shape MIGRATION (Metaclass>>addSlot: -> copyObject:to:), which copies each instance into a new-shape object and swaps identities. become: is NOT it either: a becomeForward: between two Arrays, with a third object holding a reference, answers #BBB identically with the knob on and off. RULED OUT BY TEST, do not re-try: the allocator (profiling), the linear first-fit walk, locality, waiting on a semaphore, free chunks reaching the image via allObjectsDo or objectAfter, the prevEnd base-vs-header overshoot, gaps written over live memory, marking/collection of free-list objects (200 held Arrays survive a fullGC intact), and become:. NEW EVIDENCE 2026-08-23 via PHARO_FREECHUNK_REFS: (a) the bad address is NEVER handed out -- 4403 free-list allocations in the failing run, none at 0x700008ba20, so it is not a double-free; (b) the stale pointer sits in the RUNNING METHOD'S temps and stack slots (temp[0], temp[4], stack @51/@55 under copyObject:to:) plus one heap object; (c) classIndex 0 is a FREE CHUNK, not a forwarder (ForwardedClassIndex==8). CORRECTION: an earlier note here claimed that without the knob compaction never MOVES anything (from a `tomove=0` plan line). That was a different scenario -- a fresh fullGC on a just-loaded image. Measured on the actual workload, compaction relocates about the same either way: freelist=off inplace=739642 tomove=16937, freelist=on inplace=739642 tomove=16935. So relocation IS exercised without the knob and works; the knob does not newly enable it. The difference must be WHERE the objects live (low, in former gaps), not WHETHER anything moves. Next: updatePointersAfterCompact (ObjectMemory.cpp:5005) versus a reference held only in an active method's temps.
