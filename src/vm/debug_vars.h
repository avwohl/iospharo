...
DEBUG_BOOL(PHARO_T1_X86_INLINE_J2J)         // x86 WIP: opt-in self-recursive inline-J2J (default off). Send-site save-push + per-return pop/restore/branch, bypassing the C++ activate/resume round-trip for self-recursive JIT->JIT calls. Knob-gated until validated; arm64 unaffected.
