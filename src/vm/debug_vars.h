...
DEBUG_BOOL(PHARO_T1_X86_XMETHOD_SENDS)      // Increment 2 (opt-IN, x86): admit SEND-BEARING cross-method inline-J2J callees (numICEntries>0, up to PHARO_T1_XMETHOD_MAX_IC sites), replacing the canSkipJ2JSave (numIC==0 leaf-only) admit gate of Increment 1. Still excludes canBailMidMethod / hasPrimPrologue / hasHeapWrites (the genuine mid-method-C++-reentry shapes). Closes the cfibt/cfibs resume-thrash gap (J2J callees that make an inner send: 672ms->target ~cfibx; box #18 2026-06-16). The materialize bugs that justified leaf-only are fixed (j2jDepth 2026-06-09, literalsCache 2026-06-16). arm64 compiles out (x86-only emit). docs/x86-cog-gap.md.
