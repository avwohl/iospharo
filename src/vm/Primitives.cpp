...
PrimitiveResult Interpreter::primitiveSnapshot(int argCount) {
    ...
    // Open a log file next to the image so Tim can read it from TestFlight
    std::string logPath = savePath;
    auto lastSlash = logPath.rfind('/');
    if (lastSlash != std::string::npos)
        logPath = logPath.substr(0, lastSlash + 1) + "snapshot.log";
    else
        logPath = "snapshot.log";
    FILE* logFile = fopen(logPath.c_str(), "a");  // append
    ...
    {
        char buf[512];
        snprintf(buf, sizeof(buf), "[SNAPSHOT] primitiveSnapshot called (argCount=%d) saving to: %s",
                 argCount, savePath.c_str());
        log(buf);
    }
    ...
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "[SNAPSHOT] Phase 4: Full GC (heap=%zuMB)...",
    ...
    if (!stackpOop.isSmallInteger()) {
        log("[SNAPSHOT] Bad stackp in active context");
        if (logFile) fclose(logFile);
    ...
    log("[SNAPSHOT] Phase 6: Writing image...");
    ...
    memory_.storePointer(ProcessSuspendedContextIndex, activeProcess, memory_.nil());
    log("[SNAPSHOT] Phase 8: Resuming execution...");
    executeFromContext(activeCtx);

    if (!result.success) {
        char buf[512];
        snprintf(buf, sizeof(buf), "[SNAPSHOT] Save failed: %s", result.error.c_str());
        log(buf);
    }
...
static intptr_t bitBltField(ObjectMemory& memory, Oop bitBlt, size_t index) {
    ...
    if (field.isObject()) {
        ...
        // LargePositiveInteger / LargeNegativeInteger (bytes object, small slot count)
        if (hdr->isBytesObject() && hdr->slotCount() <= 2) {
            size_t byteSize = hdr->byteSize();
            if (byteSize > 0 && byteSize <= 8) {
                int64_t val = 0;
                std::memcpy(&val, hdr->bytes(), byteSize);
                // Check class hash to distinguish positive from negative
                // LargeNegativeInteger has a different class — negate the value
                // For now, treat all as positive (coordinates are usually positive)
                intptr_t result = static_cast<intptr_t>(val);
                memory.storePointer(index, bitBlt, Oop::fromSmallInteger(result));
                return result;
            }
...
PrimitiveResult Interpreter::primitiveCopyBits(int argCount) {
    ...
    // Reference VM: nil width/height default to dest form dimensions
    if (width == 0 && memory_.fetchPointer(BBWidth, bitBlt).isNil()) width = destWidth;
    if (height == 0 && memory_.fetchPointer(BBHeight, bitBlt).isNil()) height = destHeight;
        ...
        for (intptr_t y = 0; y < height; y++) {
            ...
            switch (combinationRule) {
                ...
                case 4: // OR NOT
                    for (intptr_t x = 0; x < width; x++) row[x] |= ~fill;
                    break;
                case 24: { // alpha blend fill (source-over, non-premultiplied)
                    uint32_t sa = (fill >> 24) & 0xFF;
                    if (sa == 255) {
                        for (intptr_t x = 0; x < width; x++) row[x] = fill;
                    } else if (sa > 0) {
                        ...
                        for (intptr_t x = 0; x < width; x++) {
                            uint32_t d = row[x];
                            uint32_t dAlpha = (d >> 24) & 0xFF;
                            // Proper /255: add correction term before shift
                            uint32_t t_rb = fillRB * sa + (d & 0xFF00FF) * invSa + 0x800080;
                            uint32_t rb = ((t_rb + ((t_rb >> 8) & 0xFF00FF)) >> 8) & 0xFF00FF;
                            uint32_t t_g = fillG * sa + (d & 0x00FF00) * invSa + 0x008000;
            ...
            switch (combinationRule) {
                ...
                case 24: { // alpha blend (source-over, non-premultiplied)
                    for (intptr_t x = 0; x < width; x++) {
                        ...
                        uint32_t dAlpha = (d >> 24) & 0xFF;
                        // Proper /255: add correction term before shift
                        uint32_t t_rb = (s & 0xFF00FF) * sa + (d & 0xFF00FF) * invSa + 0x800080;
                        uint32_t rb = ((t_rb + ((t_rb >> 8) & 0xFF00FF)) >> 8) & 0xFF00FF;
                        uint32_t t_g = (s & 0x00FF00) * sa + (d & 0x00FF00) * invSa + 0x008000;
                        uint32_t g = ((t_g + ((t_g >> 8) & 0x00FF00)) >> 8) & 0x00FF00;
                        uint32_t outAlpha = sa + ((dAlpha * invSa + 127) / 255);
                        dstRow[x] = rb | g | (outAlpha << 24);
                    }
            ...
            for (intptr_t x = 0; x < width; x++) {
                ...
                switch (combinationRule) {
                    ...
                    case 7: // OR
                        dstRow[x] |= srcPixel;
                        break;
                    case 25: // pixPaint: zero → keep dest; non-zero → replace
                    ...
                    case 24: { // alpha blend (source-over, non-premultiplied)
                        ...
                        uint32_t d = dstRow[x];
                        uint32_t dAlpha = (d >> 24) & 0xFF;
                        uint32_t t_rb = (srcPixel & 0xFF00FF) * sa + (d & 0xFF00FF) * invSa + 0x800080;
                        uint32_t rb = ((t_rb + ((t_rb >> 8) & 0xFF00FF)) >> 8) & 0xFF00FF;
                        uint32_t t_g = (srcPixel & 0x00FF00) * sa + (d & 0x00FF00) * invSa + 0x008000;
                        uint32_t g = ((t_g + ((t_g >> 8) & 0x00FF00)) >> 8) & 0x00FF00;
                        uint32_t outAlpha = sa + ((dAlpha * invSa + 127) / 255);
                        dstRow[x] = rb | g | (outAlpha << 24);
                        break;
...
// Non-static: accessed by enterInterpreterFromCallback in Interpreter.cpp
int g_callbackSemaphoreIndex = 0;
...
// Returns the current VMCallbackContext* as an ExternalAddress, or nil if no active callback.
// The Smalltalk side uses this to access callback arguments and return type info.
...
// Called by the callback handler process after computing the return value.
