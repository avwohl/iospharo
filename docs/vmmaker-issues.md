...
side to unsigned for the comparison, so if either side were ever negative it
would become a very large positive value and the comparison would invert.

**Resolved 2026-08-16: neither side can be negative, so the conversion is
value-preserving and the emitted comparison is correct.** This entry previously
called it a genuine logic defect and left the question open; it is a false
positive. Reading `DSAPrims.c:160-222`:

  - `d1`, `d2`, `firstDigit` and `thirdDigit` are declared `unsigned char`, so
    each is 0-255. `firstTwoDigits = (firstDigit << 8) + dsaRemainder[j-1]`
    is therefore 0-65535.
  - `q` starts as either `0xFF` or `firstTwoDigits / d1`, both non-negative and
    at most 255 (Knuth's Algorithm D normalises the divisor so `d1 >= 128`,
    which also rules out the divide by zero). So `d2 * q` starts in 0-65025.
  - The correction step can decrement `q` at most twice, so the only way the
    left side could go negative is `q` reaching -1. It cannot:
      - At `q == 0` the test is `0 > (non-negative)`, which is false, so no
        decrement happens.
      - At `q == 1` the test may fire once, leaving `q == 0`; the nested test
        is then `0 > (non-negative)`, false again.
  - The right side is non-negative too. `firstTwoDigits - q*d1` is the
    remainder `firstTwoDigits mod d1` in the division branch, and
    `d1 + dsaRemainder[j-1]` in the `firstDigit == d1` branch; each decrement
    of `q` only adds `d1` to it.

So the useful fix is the cosmetic one after all: give both sides the same
signedness in the Smalltalk, so the warning stops without anyone having to
redo this proof. Casting in the generated C would not survive regeneration.
