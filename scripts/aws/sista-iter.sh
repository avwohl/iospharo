...
#
# From the worktree after committing a change on the work branch ($WORK_BRANCH,
# default jit since the x86 work merged 2026-06-17):
...
#
# Pushes $WORK_BRANCH, syncs it on the live spot box (state.env PUBLIC_IP),
...
git -C "$HERE/../.." push origin "$WORK_BRANCH" 2>&1 | tail -2

BUILD="cd ~/src/iospharo && git fetch -q origin $WORK_BRANCH && git reset -q --hard origin/$WORK_BRANCH && \
