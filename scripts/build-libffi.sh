...
make_universal() {
    ...
    if ! diff -q "${dir_a}/include/ffitarget.h" "${dir_b}/include/ffitarget.h" >/dev/null 2>&1; then
        cp "${dir_a}/include/ffitarget.h" "${install_dir}/include/ffitarget-${slice_a}.h"
        cp "${dir_b}/include/ffitarget.h" "${install_dir}/include/ffitarget-${slice_b}.h"
        {
            echo "/* Generated by build-libffi.sh: ${slice_a} and ${slice_b} need"
            echo "   different ffitarget.h, and a fat slice has only one header dir. */"
            echo "#if defined(__aarch64__) || defined(__arm64__)"
            case "${slice_a}" in *arm64*) echo "#include \"ffitarget-${slice_a}.h\"" ;;
                                 *)       echo "#include \"ffitarget-${slice_b}.h\"" ;; esac
            echo "#elif defined(__x86_64__)"
            case "${slice_a}" in *x86_64*) echo "#include \"ffitarget-${slice_a}.h\"" ;;
                                 *)        echo "#include \"ffitarget-${slice_b}.h\"" ;; esac
            echo "#else"
            echo "#error \"libffi: no ffitarget.h staged for this architecture\""
            echo "#endif"
        } > "${install_dir}/include/ffitarget.h"
        log "  ${label}: staged per-arch ffitarget.h (${slice_a}, ${slice_b})"
    fi
