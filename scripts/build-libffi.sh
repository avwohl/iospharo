...
make_universal() {
    ...
    # ffitarget.h is ARCH-SPECIFIC and lipo does not merge headers, so taking
    # slice_a's copy silently gave the other architecture the wrong ABI:
    #
    #   macos-arm64  ffitarget.h   FFI_FIRST_ABI = 0, FFI_SYSV,   default SYSV   -> 1
    #   macos-x86_64 ffitarget.h   FFI_FIRST_ABI = 1, FFI_UNIX64, default UNIX64 -> 2
    #
    # so an x86_64 build compiled FFI_DEFAULT_ABI = 1 and every ffi_prep_cif
    # answered FFI_BAD_ABI (2). Measured 2026-08-18: 77 of the x86_64 SUnit
    # sweep's errors, the whole TFCallback/FFICallback family, from
    # `[TFCB-FAIL] ffi_prep_cif status=2`. arm64 was unaffected because arm64
    # is slice_a. Ship both and dispatch on the compiler's own arch macros.
    # ffi.h needs exactly the same treatment, and not splitting it cost a
    # second, worse bug that hid until 2026-08-22.  configure substitutes
    # @FFI_EXEC_TRAMPOLINE_TABLE@ into ffi.h, and it differs by arch:
    #
    #   macos-arm64   ffi.h   #if 1  -> void *trampoline_table[_entry]
    #                                   cif@16 fun@24 sizeof(ffi_closure)=40
    #   macos-x86_64  ffi.h   #if 0  -> char tramp[FFI_TRAMPOLINE_SIZE=32]
    #                                   cif@32 fun@40 sizeof(ffi_closure)=56
    #
    # Shipping only arm64's copy meant the x86_64 VM compiled a 40-byte
    # ffi_closure, passed that size to ffi_closure_alloc, and then let the
    # x86_64 library write fun at offset 40 and user_data at 48 -- 16 bytes
    # PAST the allocation.  The trampoline read fun from the same out-of-
    # bounds slot and jumped to whatever was there.  Observed: every Athens
    # Cairo call that registers a callback crashed the x86_64 VM at
    # PC=0x33, e.g. `AthensCairoSurface extent: 100@100`.  It stayed hidden
    # only because there was no x86_64 libcairo on the build host, so those
    # calls failed at symbol lookup and never reached a closure.
    for hdr in ffitarget ffi; do
        diff -q "${dir_a}/include/${hdr}.h" "${dir_b}/include/${hdr}.h" >/dev/null 2>&1 && continue
        cp "${dir_a}/include/${hdr}.h" "${install_dir}/include/${hdr}-${slice_a}.h"
        cp "${dir_b}/include/${hdr}.h" "${install_dir}/include/${hdr}-${slice_b}.h"
        {
            echo "/* Generated by build-libffi.sh: ${slice_a} and ${slice_b} need"
            echo "   different ${hdr}.h, and a fat slice has only one header dir. */"
            echo "#if defined(__aarch64__) || defined(__arm64__)"
            case "${slice_a}" in *arm64*) echo "#include \"${hdr}-${slice_a}.h\"" ;;
                                 *)       echo "#include \"${hdr}-${slice_b}.h\"" ;; esac
            echo "#elif defined(__x86_64__)"
            case "${slice_a}" in *x86_64*) echo "#include \"${hdr}-${slice_a}.h\"" ;;
                                 *)        echo "#include \"${hdr}-${slice_b}.h\"" ;; esac
            echo "#else"
            echo "#error \"libffi: no ${hdr}.h staged for this architecture\""
            echo "#endif"
        } > "${install_dir}/include/${hdr}.h"
        log "  ${label}: staged per-arch ${hdr}.h (${slice_a}, ${slice_b})"
    done
...
for slice in ios-device-arm64 ios-sim-universal catalyst-universal macos-universal; do
    ...
    # Also check lib/libffi-*/include for ffi.h and ffitarget.h -- but only if
    # include/ did not already provide them.  This `find | cp` used to run
    # unconditionally and would happily overwrite the per-arch DISPATCHER
    # headers written above with whichever single-arch copy it stumbled on.
    for hdr in ffi ffitarget; do
        [ -f "${install_dir}/Headers/${hdr}.h" ] && continue
        found=$(find "${install_dir}" -name "${hdr}.h" | head -1)
        [ -n "$found" ] && cp "$found" "${install_dir}/Headers/"
    done
done
