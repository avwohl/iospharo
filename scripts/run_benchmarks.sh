...
if $RUN_OURS; then
    ...
    #
    # 2026-05-17: arm64 used to need PHARO_NO_SISTA_DO_SPLICE=1 to
    # avoid an ~80% hang at runCollect (deferred.md A5 — scheduler
    # race in SessionManager-startUp:-forked-at-high-priority).
    # After the 2026-05-18 inline-J2J + bytecode-coverage shipping,
    # 8/8 runs complete cleanly without the workaround.  Removing.
    # PHARO_KEEP_SISTA_DO_SPLICE=1 (the historic opt-out flag) still
    # works as a no-op; setting PHARO_NO_SISTA_DO_SPLICE=1 reinstates
    # the workaround if anyone hits the hang again.
